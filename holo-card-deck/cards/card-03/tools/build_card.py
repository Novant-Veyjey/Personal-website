...
# Compositor glow.
try:
    ct=bpy.data.node_groups.new('全息合成节点','CompositorNodeTree'); scene.compositing_node_group=ct
    ct.interface.new_socket('Image',in_out='OUTPUT',socket_type='NodeSocketColor')
    composite_type='NodeGroupOutput'
except Exception:
    scene.use_nodes=True; ct=scene.node_tree; composite_type='CompositorNodeComposite'
ct.nodes.clear(); rl=node(ct,'CompositorNodeRLayers','渲染层',0,0); gl=node(ct,'CompositorNodeGlare','辉光 · 高质量',250,0); gl.inputs['Type'].default_value='Fog Glow'; gl.inputs['Quality'].default_value='High'; gl.inputs['Threshold'].default_value=1.5; gl.inputs['Size'].default_value=.45; co=node(ct,composite_type,'最终图像',510,0); link(ct,rl,'Image',gl,'Image'); link(ct,gl,'Image',co,'Image')
...
try:
    ...
    print('Skipped user preference save:',error)
scene.frame_set(25)
...
scene['制作说明']='怪奇物语主题全息收藏卡 · 霍金斯档案。按空格播放转动，可调节镭射与分层景深。'
scene['素材来源']='基于用户提供的五张参考图，重建为背景、主体、线稿、文字与特效层。'
