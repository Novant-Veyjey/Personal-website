...
function backTexture() {
  ...
  ctx.fillText(config.collection || "THE UPSIDE DOWN", 512, 245);
  ctx.font = '34px "Songti SC", serif';
  ctx.fillText(config.subtitle || config.title, 512, 1020);
  ctx.font = "18px Arial";
  ctx.fillText(config.edition || "SIGNAL ARCHIVE", 512, 1337);
  ctx.beginPath();
  ctx.moveTo(460, 1113);
  ctx.lineTo(564, 1113);
  ctx.stroke();
...
async function init() {
  ...
  document.title = config.title + " · THE UPSIDE DOWN";
  for (const [id, key] of [
    ["card-title", "title"],
    ["subtitle", "subtitle"],
...
Promise.race([
  ...
  if (settled) return;
  console.error(error);
  try { fallback3D(error); } catch (fallbackError) { fail("作品暂时无法加载。\n" + fallbackError.message); }
});
